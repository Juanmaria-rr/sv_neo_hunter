...
SPEC: dict[str, tuple] = {
    ...
    "test": ("stage 8", "RNA — direct read counting", "annotation",
        ...
        "**The ARCHITECTURE of the alteration — deletion, chimeric junction or "
        "insertion — which is what decides how it can be detected at all.** A read "
        "crossing the junction does not look the same in the three cases, because the "
        "alteration has done something physically different in each. **Deletion**: the "
        "read aligns continuously but skips the missing stretch. **Chimeric junction**: "
        "no continuous alignment is possible at all, so the aligner splits the read in "
        "two. **Insertion**: the added bases sit inside the read with nothing in the "
        "reference to match. So the detection method is not a choice made on the "
        "evidence — it is a consequence of the architecture, derived from the "
        "coordinates before any read is looked at. One of "
        f"`sizegap` (a DELETION: one chromosome, resolvable size >= "
        f"{C.MIN_TESTABLE_GAP_SIZE} bp), `chimeric` (a CHIMERIC JUNCTION: "
        f"inter-chromosomal, or BND/TRA), `insertion` (an INSERTION of at least "
        f"{C.MIN_INSERT_LEN} bp), or `none`. "
